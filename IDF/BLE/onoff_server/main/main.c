...
{
    switch (event) {
    case ESP_BLE_MESH_PROV_REGISTER_COMP_EVT:       // 初始化BLE Mesh供应能力和内部数据信息完成事件
        ESP_LOGI(TAG, "ESP_BLE_MESH_PROV_REGISTER_COMP_EVT, err_code %d", param->prov_register_comp.err_code);  // 指示BLE Mesh初始化的结果
        break;
    case ESP_BLE_MESH_NODE_PROV_ENABLE_COMP_EVT:    // 启用节点配置功能完成事件
        ESP_LOGI(TAG, "ESP_BLE_MESH_NODE_PROV_ENABLE_COMP_EVT, err_code %d", param->node_prov_enable_comp.err_code);    // 使能BLE Mesh设备的结果
        break;
    case ESP_BLE_MESH_NODE_PROV_LINK_OPEN_EVT:      // 建立一个BLE Mesh 连接事件
        ESP_LOGI(TAG, "ESP_BLE_MESH_NODE_PROV_LINK_OPEN_EVT, bearer %s",
            param->node_prov_link_open.bearer == ESP_BLE_MESH_PROV_ADV ? "PB-ADV" : "PB-GATT");
        break;
    case ESP_BLE_MESH_NODE_PROV_LINK_CLOSE_EVT:     // 关闭一个BLE Mesh 连接事件
        ESP_LOGI(TAG, "ESP_BLE_MESH_NODE_PROV_LINK_CLOSE_EVT, bearer %s",
            param->node_prov_link_close.bearer == ESP_BLE_MESH_PROV_ADV ? "PB-ADV" : "PB-GATT");
        break;
    case ESP_BLE_MESH_NODE_PROV_COMPLETE_EVT:       // 配置完成的事件
    ...
    case ESP_BLE_MESH_NODE_PROV_RESET_EVT:          // 配置重置事件
        ...
        break;
    case ESP_BLE_MESH_NODE_SET_UNPROV_DEV_NAME_COMP_EVT:    // 设置未分配的设备名称完成事件
...
static esp_err_t ble_mesh_init(void)
{
    ...
    esp_ble_mesh_register_config_server_callback(example_ble_mesh_config_server_cb);    // 注册 BLE Mesh Config Server 模型回调。
    esp_ble_mesh_register_generic_server_callback(example_ble_mesh_generic_server_cb);  // 注册 BLE Mesh 通用服务器模型回调

    err = esp_ble_mesh_init(&provision, &composition);          // 初始化 BLE Mesh 模块。
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize mesh stack (err %d)", err);
        return err;
    }

    err = esp_ble_mesh_node_prov_enable(ESP_BLE_MESH_PROV_ADV | ESP_BLE_MESH_PROV_GATT);    // 启用特定的供应载体以使设备准备好供应。
    if (err != ESP_OK) {
